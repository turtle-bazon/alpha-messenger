...
Поднимет БД, сервер (http://localhost:3100) и веб-клиент (http://localhost:5273). Миграции применяются автоматически. Host-порты prod смещены (+100), чтобы стек не конфликтовал с dev (тот на 3000/5173) — оба можно держать одновременно.
...
Выведет ссылку `http://localhost:5273/register?invite=<КОД>` (для dev — `5173`) — откройте её в браузере. Чтобы проверить переписку вживую, создайте второй invite и войдите вторым пользователем в другом окне/профиле браузера. (Код кладётся в БД через работающий контейнер — Node не нужен.)
