...
- **Несколько провайдеров**: на мобильном устройстве может быть несколько UP-приложений (ntfy, UnifiedPush, etc.) — нужно показывать выбор провайдера, если их несколько. UI выбора — рядом с настройками push-уведомлений (в `PushWarningBanner` или отдельном блоке). В будущем перенести в экран настроек (#67).
