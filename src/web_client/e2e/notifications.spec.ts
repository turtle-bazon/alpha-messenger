...
// Браузерные попапы должны работать «из коробки»: приложение само запрашивает
// системное разрешение при входе (а не прячет запрос за тумблером), если оно
// ещё не дано и не запрещено.
test('приложение само запрашивает разрешение на уведомления при входе', async ({
  browser,
}) => {
  ...
  // Разрешение запрошено автоматически, без открытия меню настроек.
  await expect
    .poll(() =>
      page.evaluate(
        () => (window as unknown as { __reqCount: number }).__reqCount,
  ...
  // Тумблер браузерных уведомлений честно показывает «выключено», пока разрешение
  // не выдано (permission остался 'default').
