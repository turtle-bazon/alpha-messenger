...
// при первом входе показывается модальный диалог, запрос разрешения — по клику (user gesture).
test('при default permission показывается диалог и запрашивается разрешение по клику', async ({
  page,
}) => {
  ...
  // Диалог виден при первом входе (ключей нет, permission = default)
  ...
  // Клик «Разрешить» — вызывается requestPermission (user gesture). Модалка
  // блокирует фон, поэтому сперва закрываем её, а уже потом лезем в меню настроек.
...
// диалог скрывается, browser фиксируется в '0'.
test('при default permission нажатие «Не сейчас» отключает браузерные уведомления', async ({
  page,
}) => {
  await page.addInitScript(() => {
    Object.defineProperty(Notification, 'permission', {
